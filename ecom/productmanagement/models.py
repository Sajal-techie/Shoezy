...
class Product(models.Model):
    ...
    def offer_price(self):
        ...
        if product_offers.exists() and brand_offers.exists():
            ...
            discount_percentage = round(((int(self.original_price) - int(self.selling_price)) / float(self.original_price)) * 100)
            return self.selling_price,discount_percentage
        
        offer_price = round( int(self.selling_price) * ( 1 - max_offer / 100 ),2) 
        discount_percentage = round(((int(self.original_price) - offer_price) / float(self.original_price)) * 100)
